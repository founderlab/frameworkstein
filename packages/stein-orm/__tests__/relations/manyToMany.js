...
describe('ManyToMany', () => {
  ...
  it('Can add a m2m relation with `link` and remove with `unlink` with the same model', async () => {
    ...
    const linkData = {owner_id: childOwner.id, reverse_id: parentOwner.id}
    const link = await SelfOwner.link('parentOwners', linkData)

    expect(link).toBeTruthy()
    expect(link.data.owner_id).toBe(childOwner.id)
    expect(link.data.reverse_id).toBe(parentOwner.id)
