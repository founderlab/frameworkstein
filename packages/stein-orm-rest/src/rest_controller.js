...
module.exports = (RESTController = (function() {
  RESTController = class RESTController extends (require('./lib/json_controller')) {
    ...
    constructor(app, options) {
      ...
      if (this.cache) {
        ...
        if (this.cache.createHash && !this.cache.hash) {
          ...
          if (!this.cache.hash) { this.cache.hash = this.route; }
        }
      }
    ...
    clearCache(callback) {
      ...
      for (let hash_key of Array.from(hash_keys)) {
    ...
    render(req, json, callback) {
      ...
      try { template_name = JSON.parse(template_name); } catch (error) {} // remove double quotes
