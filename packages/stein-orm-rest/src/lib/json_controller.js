...
module.exports = (JSONController = (function() {
  JSONController = class JSONController {
    ...
    sendStatus(res, status, message) { res.status(status); if (message) { return res.json({message}) }  return res.json({})  }
    sendError(res, err) {
      const { req } = res
    ...
    requestValue(req, key) { if (_.isFunction(req[key])) { return req[key]() }  return req[key]  }
