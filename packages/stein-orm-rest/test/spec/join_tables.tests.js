...
_.each(BackboneORM.TestUtils.optionSets(), (exports = function(options) {
  ...
  return describe(`Many to Many (${options.$tags}, framework: ${options.app_framework.name})`, () => {
    let mockApp,
      Owner
    ...
    beforeEach((callback) => {
      ...
      queue.defer((callback) => {
        ...
        }, (err, models) => { MODELS.reverse = models; return callback(err) }),
        )
        create_queue.defer(callback => Fabricator.create(Owner, BASE_COUNT, {
          name: Fabricator.uniqueId('owners_'),
          created_at: Fabricator.date,
        }, (err, models) => { MODELS.owner = models; return callback(err) }),
        )
      ...
      queue.defer((callback) => {
        ...
        for (const owner of Array.from(MODELS.owner)) {
          (owner => save_queue.defer((callback) => {
            const reverses = [MODELS.reverse.pop(), MODELS.reverse.pop()]
            // console.log "linking #{owner.id} to #{reverses[0]?.id}, #{reverses[1]?.id}"
            owner.set({reverses})
            for (const reverse of Array.from(owner.get('reverses').models)) { MODELS_JSON.push({owner_id: owner.id, reverse_id: reverse.id}) } // save relations
    ...
    it.skip('Can include related (two-way hasMany) models', (done) => {
      ...
      return Owner.cursor({$one: true}).include('reverses').toJSON((err, test_model) => {
        assert.ok(!err, `No errors: ${err}`)
    ...
    it.skip('Can query on related (two-way hasMany) models', done =>
      Reverse.findOne((err, reverse) => {
        ...
        return Owner.cursor({'reverses.name': reverse.get('name')}).toJSON((err, json) => {
    ...
    it.skip('Can query on related (two-way hasMany) models with included relations', done =>
      Reverse.findOne((err, reverse) => {
        assert.ok(!err, `No errors: ${err}`)
        assert.ok(reverse, 'found model')
        return Owner.cursor({'reverses.name': reverse.get('name')}).include('reverses').toJSON((err, json) => {
          const test_model = json[0]
          assert.ok(!err, `No errors: ${err}`)
          assert.ok(test_model, 'found model')
          assert.ok(test_model.reverses, 'Has related reverses')
    ...
    it.skip('Clears its reverse relations on delete when the reverse relation is loaded', done =>
      Owner.cursor({$one: true, $include: 'reverses'}).toModels((err, owner) => {
        ...
        return owner.get('reverses', (err, reverses) => {
          assert.ok(!err, `No errors: ${err}`)
          assert.ok(reverses, 'found model')

          return owner.destroy((err, owner) => {
            assert.ok(!err, `No errors: ${err}`)

            return Owner.relation('reverses').join_table.find({owner_id: owner.id}, (err, null_reverses) => {
              assert.ok(!err, `No errors: ${err}`)
    ...
    return it.skip('Can query on a ManyToMany relation by related id', done =>
      Owner.findOne((err, owner) => {
        assert.ok(!err, `No errors: ${err}`)
        assert.ok(owner, 'found model')
        return Reverse.cursor({owner_id: owner.id}).toModels((err, reverses) => {
          assert.ok(!err, `No errors: ${err}`)
