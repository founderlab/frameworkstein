 ...
 */
let _, Backbone, exports, Fabricator, JSONUtils, Utils;
...
_.each(BackboneORM.TestUtils.optionSets(), (exports = function(options) {
  ...
  return describe(`RestController (sorted: false, ${options.$tags}, framework: ${options.app_framework.name})`, function() {
    ...
    beforeEach(callback =>
      Utils.resetSchemas([Flat, Related], function(err) {
        ...
        return Fabricator.create(Flat, BASE_COUNT, {
          ...
          return Flat.find({$ids: _.pluck(models, 'id')}, function(err, models) { // reload models in case they are stored with different date precision
            ...
            return Fabricator.create(Related, BASE_COUNT, {
              ...
              return Related.find({$ids: _.pluck(models, 'id')}, function(err, models) { // reload models in case they are stored with different date precision
                if (err) { return callback(err); }
    ...
    describe('create', function() {
      ...
      return it('should clear a related models cache on create', function(done) {
        const app = APP_FACTORY();
        ...
        queue.defer(callback =>
          Related.exists({id: related_id}, function(err, exists) {
            assert.ok(!err, `no errors: ${err}`);
    ...
    describe('update', function() {
      ...
      return it('should clear a related models cache on update', function(done) {
        const app = APP_FACTORY();
        const cache_options = _.extend({}, CACHE_OPTIONS, {cascade: [RELATED_ROUTE]});
        const controller = new RestController(app, {model_type: Flat, route: ROUTE, default_template: null, cache: cache_options});
        const related_controller = new RestController(app, {model_type: Related, route: RELATED_ROUTE, default_template: null, cache: CACHE_OPTIONS});
    ...
    return describe('delete', function() {
      ...
      return it('should clear a related models cache on delete', function(done) {
        const app = APP_FACTORY();
        const cache_options = _.extend({}, CACHE_OPTIONS, {cascade: [RELATED_ROUTE]});
        const controller = new RestController(app, {model_type: Flat, route: ROUTE, default_template: null, cache: cache_options});
        const related_controller = new RestController(app, {model_type: Related, route: RELATED_ROUTE, default_template: null, cache: CACHE_OPTIONS});
